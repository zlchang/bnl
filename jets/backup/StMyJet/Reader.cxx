...
//Reader::Reader(StJetEvent *jetevnt, StJetEvent *parton, StJetEvent *particle, StJetSkimEvent *skimevnt, StUeEvent *ueocevnt)
...
void Reader::fillMyJetQA(int numJet, MyJet *jetmax[], string trgmax[], map<string, MyJetQA*> mapJetQa, bool flag, float weight)
{
  ...
  for(int ijet = 0; ijet < numJet; ijet++){
    ...
    mapJetQa[trgmax[ijet]]->FillJetMult(jtwrmult, jtrkmult, weight);
...
void Reader::fillMyJetMc(int numJet, MyJet *jetmax[], string trgmax[], map<string, MyJetMC*> mapJetMc, bool flag, float weight)
{
  ...
  for(int ijet = 0; ijet < numJet; ijet++){
    ...
    int partonCounter = 0;
    StJetCandidate *parton = FindParJet(pj->jet(), mJetPartonEvent, partonIndex[ijet], 1.4, partonCounter);
    if(parton){
    ...
    StJetCandidate *particle = FindParJet(pj->jet(), mJetParticleEvent, particleIndex[ijet], 1.4, particleCounter);
    if(particle){
  ...
  if(vz < 90. && vz > -90.){
    ...
    for(int iJet = 0; iJet < NJets; iJet++){
      ...
      if(eta > -0.9 && eta < 0.9){
        ...
        float waLL = pythia->partonALL();//aLLNNPDF(0);
