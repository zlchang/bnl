...
{
  public:
  //Reader(StJetEvent *jetevnt, StJetEvent *parton, StJetEvent *particle, StJetSkimEvent *skimevnt, StUeEvent *ueocevnt);
  Reader(StJetEvent *jetevnt = 0x0, StJetEvent *parton = 0x0, StJetEvent *particle = 0x0, StJetSkimEvent *skimevnt = 0x0, StUeOffAxisConesEvent *ueocevnt = 0x0);
  int Init();
  int Make(int iEvent);
