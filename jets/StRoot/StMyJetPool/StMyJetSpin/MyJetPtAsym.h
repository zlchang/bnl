...
class MyJetPtSpin : public TObject
{
 public:
  ...
  void AddMult(MyJetSpin *jetspin);
  void AddMultUe(MyJetSpin *jetspin);
	  ...
	  for(int ibin = 1; ibin <= Nbins; ibin++){
	    ...
	    for(int ispin = 0; ispin < 4; ispin++){
	      ...
	      error[ispin] = h[ispin]->GetBinError(ibin);
              //if((int) h[ispin]->GetBinContent(ibin) == 1) error[ispin] = counts[ispin];
	      //if(ibin == 15) cout<<ispin<<" "<<counts[ispin]<<" "<<error[ispin]<<endl;
	    }
	    float num = 0.;
	    float den = 0.;
	    float num_err = 0.;
	    ...
	    den = counts[1] + counts[2];
            //if(ibin == 15) cout<<num_err<<" "<<den<<endl;
