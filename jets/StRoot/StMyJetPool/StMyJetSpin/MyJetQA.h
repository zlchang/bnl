...
class MyJetQA : public TObject
{
 public:
  ...
  MyJetQA(const char* name)
      ...
      if(name)
	{
	  ...
	  hjetmult = new TH2F(Form("%sJetMult", name), ";mult", yNPtBins, yPtBins, 200, 0, 200);
	  hjettrkmult = new TH2F(Form("%sJetTrackMult", name), ";track mult", yNPtBins, yPtBins,100, 0, 100);
	  hjettrkpt = new TH1F(Form("%sJetTrackPt", name), ";track p_{T} [GeV]", 100, 0, 100);
	  hjettwrmult = new TH2F(Form("%sJetTowerMult", name), ";tower mult", yNPtBins, yPtBins, 100, 0, 100);
	  ...
	  hjettwrmultprof = new TProfile(Form("%sJetTowerMultVsPt", name), ";p_{T} [GeV]; tower mult", yNPtBins, yPtBins);
	  hjettrkmultprof = new TProfile(Form("%sJetTrackMultVsPt", name), ";p_{T} [GeV]; track mult", yNPtBins, yPtBins);
  ...
  ~MyJetQA(){
      ...
      if(hjettrkmultprof != NULL) delete hjettrkmultprof;
      if(hjettrkpt != NULL) delete hjettrkpt;
      if(hjettwrmult != NULL) delete hjettwrmult;
      if(hjettwrmultprof != NULL) delete hjettwrmultprof;
  ...
  TH2F *GetJetMult() const{
    return hjetmult;
  }	
  TH2F *GetJetTrkMult() const{
    return hjettrkmult;
  }	
  TProfile *GetJetTrkMultProf() const{
    return hjettrkmultprof;
  }
  ...
  TH2F *GetJetTwrMult() const{
    return hjettwrmult;
  }
  TProfile *GetJetTwrMultProf() const{
    return hjettwrmultprof;
  }
  //tprofiles	
  ...
  void FillJetMult(float pt, float twrmult, float trkmult, float w = 1.)
  {
    hjetmult->Fill(pt, twrmult+trkmult, w);
    hjettwrmult->Fill(pt, twrmult,w);
    hjettrkmult->Fill(pt, trkmult,w);
    hjettwrmultprof->Fill(pt, twrmult,w);
    hjettrkmultprof->Fill(pt, trkmult,w);
  }
 ...
 private:
  ...
  TProfile *hjetrtprof;
  TProfile *hjetptprof;
  TProfile *hjettrkmultprof;
  TProfile *hjettwrmultprof;
  //UE
  TH2F *hjetptarea;
  TProfile *hjetdptue;
