...
{
  return (bx < 30 || bx > 39 ) && bx < 110 && bx < 120;
}
...
StJetCandidate *FindParJet(StJetCandidate* jet, StJetEvent *evntpar, int &index, float range)
{
  float eta = jet->eta();
  float phi = jet->phi();
...
float smearJetPt(MyJet *myjet, bool ue, bool smear)
{
  ...
  float dpt = myjet->dpt();
  float rt = myjet->rt();
  float rtue = myjet->rtUe();

  float spt = pt+pt*smearPtFrac(rt)*smear;
  float sdpt = dpt+dpt*smearPtFrac(rtue)*smear;
  spt = spt - sdpt*ue;
  //cout<<"spt: "<<spt<<" ue: "<<ue<<" smear: "<<smear<<endl;
  return spt;
}
string JetCategory(MyJet *myjet, StJetSkimEvent *skimevnt, map<string, int>mapTrigShouldFired, map<string, int>mapTrigDidFired, JetPars pars, bool flag, bool smear, int th_plus)
{
  string trgname("Out");
  float pt = smearJetPt(myjet, flag, smear);
  
  StJetCandidate *jetCnd = myjet->jet();
  float minjp0 = pars.jp0minpt;
  float maxjp0 = pars.jp0maxpt;
  float minjp1 = pars.jp1minpt;
  float maxjp1 = pars.jp1maxpt;
  float minjp2 = pars.jp2minpt;
  if(pt > minjp2){
...
//old version
string JetCategory(MyJet *myjet, StJetSkimEvent *skimevnt, map<string, int>mapTrigShouldFired, map<string, int>mapTrigDidFired, bool flag, int th_plus)
{
  string trgname("Out");
  float pt = myjet->pt();
