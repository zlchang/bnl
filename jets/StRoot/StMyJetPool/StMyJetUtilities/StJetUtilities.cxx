...
TH2D *getVariance(const TH2D *h, const TH2D *h2d, bool norm)
{
  TAxis *xaxis = h->GetXaxis();
  ...
  TH2D *hh = new TH2D(Form("%sVar%d", h2d->GetName(), norm), "", nx*ny, 0, nx*ny, nx*ny, 0, nx*ny);
  TArrayD arr(nx*ny);
  for(int ix = 1; ix <= xaxis->GetNbins(); ix++){
    for(int iy = 1; iy <= yaxis->GetNbins(); iy++){
      double eh = h->GetBinError(ix, iy);
      int jx = ix+nx*(iy-1);
      double eh2 = h2d->GetBinError(jx, jx);
      arr[jx-1] = eh*eh + eh2*eh2;
    }
  }
  for(int ix = 1; ix <= nx*ny; ix++){
    for(int iy = 1; iy <= nx*ny; iy++){
      double eh2 = h2d->GetBinError(ix, iy);
      double content = TMath::Power(eh2, 2);
      double var = content;
      if(iy == ix) { double err = h->GetBinError((ix-1)%nx+1, (ix-1)/nx+1); var += err*err;}
