Int_t readMyJetCrsMc(const char* filelist="pt11_15.0.list", const char* outfile="pt11_15.0.crs.root")
{
  ...
  ReaderCrs *reader = new ReaderCrs(jetevnt, parevnt, particleevnt, skimevnt, ueocevnt);
  //ReaderCrs *reader = new ReaderCrs(jetevnt, parevnt, particleevnt, skimevnt, 0x0);
  ...
  //for(Int_t iEvt = 0; iEvt < 10; iEvt++)
    {
      ...
      uechn.GetEntry(iEvt);
      //if(skimevnt->eventId() != jetevnt->eventId()){ cout<<"skim:"<<skimevnt->eventId()<<" jet:"<<jetevnt->eventId()<<endl;  continue;}
      if(skimevnt->eventId() != jetevnt->eventId() || ueocevnt->eventId() != jetevnt->eventId()){ cout<<"skim:"<<skimevnt->eventId()<<" jet:"<<jetevnt->eventId()<<" ueoc:"<<ueocevnt->eventId()<<endl;  continue;}
