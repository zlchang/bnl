...
if (  "$SLURM_JOB_PARTITION" =~ *"chos"  ) then  
    ...
    # Georg fix for tcsh
    setenv LD_LIBRARY_PATH .sl64_gcc482/lib:/usr/common/usg/software/gcc/4.8.2/lib:/usr/common/usg/software/java/jdk1.7.0_60/lib:/usr/common/usg/software/gcc/4.8.2/lib64:/usr/common/usg/software/mpc/1.0.3/lib/:/usr/common/usg/software/gmp/6.0.0/lib/:/usr/common/usg/software/mpfr/3.1.3/lib/:$LD_LIBRARY_PATH
    echo
    ...
    echo  starting new-r4s PATH_DAQ=$PATH_DAQ, daqN=$daqN, execName=$EXEC_NAME, OUT_DIR=$OUT_DIR, workerName=`hostname -f`, startDate=`date`
    ...
    echo `date`" Fire: $EXEC_NAME for daqN=$daqN"
    ls -l .sl64_gcc482/lib/
    pwd
    #/usr/bin/time -v  $EXEC_NAME -b -q bfc.C\($NUM_EVE,\"$BFC_String\",\"$daqN\"\) >> $LOG_PATH/r4s-${SLURM_JOB_ID}.log >>& $LOG_PATH/r4s-${SLURM_JOB_ID}.err
    /usr/bin/time -v  $EXEC_NAME -b -q $MACRO_NAME\(\"$daqN\",\"$outfile\"\) 
#> $LOG_PATH/r4s-${SLURM_JOB_ID}.log >& $LOG_PATH/r4s-${SLURM_JOB_ID}.err
    echo `date`" completed job $daqN  , save results to "$OUT_DIR
    ls -l
    time cp $outfile $OUT_DIR
    echo 'copy done '`date`
