...
const char* datafile = "run12.final.v2.list.crr.v1.asym.root")
{
  ...
  TH1D *hjet = (TH1D*) mFileData->Get(Form("%sJetPt", trg[i]));
  if(!hjet) { Info("Read", Form("histogram %sJetPt not found", trg[i])); return 0;}
  TH2D *htwojet = (TH2D*) mFileData->Get(Form("%sTwoJetPt", trg[i]));
  if(!htwojet) { Info("Read", Form("histogram %sTwoJetPt not found", trg[i])); return 0;}
  ...
  cout<<"response with covariance: "<<endl;
  rescov.Print("all");

  TMatrixD bcov = rucov*b;
  //Info("", "new detector response");
  cout<<"b with covariance:"<<endl;
  bcov.Print("all"); 
  //
